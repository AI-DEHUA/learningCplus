...
void test01()
{
	...
	printVector(v1);
	//插入
	v1.insert(v1.begin(), 100);    //注意要用到迭代器
	printVector(v1);
...
如果想要只读状态，需要加关键字 `const`   `void printDeque(const deque<int>& d)`，迭代器也需要同时改变
...
for_each   函数对象无返回值，参数是引用

transform 函数对象有返回值，参数一般不使用引用
...
void test01() {
	...
	cout << endl;
    //for_each 返回一个函数对象
    print02 p1=for_each(v.begin(), v.end(), print02());
    p1.printnum();
}
...
void test01()
{
	...
	//使用回调函数
    transform(v.begin(), v.end(), vTarget.begin(), incerease);
    //使用预定义
    transform(v.begin(), v.end(), vTarget.begin(), negate<int>());
    //可以直接输出到屏幕 #include<iterator>
...
void test01()
{
	...
	if (it == v.end()) {
        ...
        int index=daitance(v.begin(),it);
        cout<<"迭代器位置"<<index<<endl;
	}
}
```
